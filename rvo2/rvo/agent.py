...
class Agent:
    ...
    def compute_new_velocity(self):
        ...
        # find nearest static neighbor
        #############################################################
        # nearestStaticNeighbor = -1                                #
        # nearestStaticNeighborDist = 100000000000000               #
        # for i in range(len(self.agent_neighbors_)):               #
        #     other = self.agent_neighbors_[i][1]                   #
        #     if not other.static_:                                 #
        #         continue                                          #
        #     relativePosition = other.position_ - self.position_   #
        #     if abs(relativePosition) < nearestStaticNeighborDist: #
        #         nearestStaticNeighborDist = abs(relativePosition) #
        #         nearestStaticNeighbor=i                           #
        #############################################################
        ...
        for i in range(len(self.agent_neighbors_)):
            ...
            relativePosition = other.position_ - self.position_
            otherVelocity = other.velocity_

            ######################################################################################
            # if other.static_ and  abs(otherVelocity)  < 0.1: # and nearestStaticNeighbor==i: # #
            #     print("stoped ", i)                                                          # #
            #     otherVelocity = self.position_ - other.position_                             # #
            #     otherVelocity = rvo_math.normalize(otherVelocity)                            # #
            ######################################################################################

            relativeVelocity = self.velocity_ - otherVelocity                                #
